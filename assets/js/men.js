...
const displayByPrice=()=>{
    homeLabel.innerText = "Men";
    if(categories=="ALL"){
        const results = productList.products.filter(product => product.category == "Men")
...
const distinctCategory = productList.products.filter(
    (element, index, self) => index === self.findIndex(
      (t) => t.type === element.type && t.category === "Men"
    )
  );

const getCategory=()=>{
   distinctCategory.forEach(cat=>{
    let countCategory = productList.products.filter(product => product.type==cat.type && product.category=="Men")
